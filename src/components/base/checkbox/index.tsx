...
interface IProps {
  ...
  helpText?: string;
  className?: string;
  classes?: Partial<ICheckboxClassNames>;

  invalid?: boolean;
  disabled?: boolean;
}

export type TCheckboxProps = IProps & React.ComponentPropsWithoutRef<"input">;

function _Checkbox(props: TCheckboxProps, ref: ForwardedRef<HTMLInputElement>) {
      ...
      <div className={classNames(classes?.inputRoot, "flex h-5 items-center")}>
        <input
          ...
          className={classNames(
            classes?.input,
            "h-5 w-5 rounded-xs !outline-none !ring-0 !ring-offset-0 bg-transparent",
            invalid
